...
namespace perception{
    ...
    {
        // YOLOv8 TensorRT output: [84, 8400] (channel-first!)
        // Row 0-3: cx, cy, w, h (all 8400 predictions)
        // Row 4-83: class scores (80 classes x 8400 predictions)
        ...
        for (int i = 0; i < num_predictions; ++i) {
            // Extract bbox (channel-first indexing)
            float cx = output[i];                           // Row 0
            float cy = output[num_predictions + i];         // Row 1
            float w = output[2 * num_predictions + i];      // Row 2
            float h = output[3 * num_predictions + i];      // Row 3

            // Find max class score
            float max_score = 0.0f;
            int max_class_id = 0;
            for (int c = 0; c < num_classes; ++c) {
            float score = output[(4 + c) * num_predictions + i];  // Rows 4-83
            if (score > max_score) {
                max_score = score;
                max_class_id = c;
            }
